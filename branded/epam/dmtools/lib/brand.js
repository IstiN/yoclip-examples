...
// A variant can swap the asset via params.logo (e.g. the dark wordmark on
// the light canvas) — see yoclip.yaml -> variants.
function epamLogo(width, opacity, margin) {
  var source = 'external:epam_logo';
  if (typeof yoclipVariant !== 'undefined' && yoclipVariant && yoclipVariant.logo) {
    source = yoclipVariant.logo;
  }
  return {
    type: 'image',
    source: source,
