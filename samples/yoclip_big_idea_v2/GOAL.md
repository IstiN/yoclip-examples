...
| 02 | 11–17 | `03_generate` | white bg, yoclip logo pops in center then glides left; "Render" types beside it; cycling words promos/shorts/docs/demos → "Render videos from code." |
| 03 | 14–17 | `04_prompt` | prompt pill types "A launch video for my app", cursor glides, clicks Render (button dips + glows) |
| 04 | 18–22 | `05_deck` | LIGHT product deck (periwinkle bg): dark navy preview card with a live golden + caption, four white feature pills sliding in from the right (teal/blue/violet/amber icons), left thumbnail rail with a mini kaleidoscope card |
| 04b | 22–32 | `05b_zoom` | dark prompt → result zoom gallery: left thumbnail rail, dark prompt UI types each request, matching result zooms from thumbnail to large preview and back — kaleidoscope, then chat agent |
| 05 | 11–17 | `02_kaleido` | portal dive into a vortex tunnel: counter-rotating neon spirals, polaroid cards with REAL golden frames spiralling into the center, low-poly 3D objects flying past; white flash out |
...
`02_kaleido` (promo window), `05_deck` (result thumbnails), `06_chat` (stats card
